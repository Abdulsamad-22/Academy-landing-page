...
export default function Programs() {
  return (
    <div className="grid grid-cols-2 md:grid-cols-[30%_30%] gap-x-[2rem] md:gap-x-[630px] gap-y-[5rem] md:gap-y-[8rem] items-top mt-[5rem] relative z-20">
      <Connector />

      {programs.map((program, index) => (
        <div key={index} className="w-full md:w-[280px] relative">
          <div className="inline-block py-2 px-3 text-[#000] border-2 border-[#c72020] mb-4">
            {program.ageGrade}
          </div>
          <p className="w-[80%] md:w-[205px]text-[1rem] text-[#161616]">
            {program.subText}
          </p>
        </div>
      ))}
